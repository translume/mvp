...
RUN pip install --no-cache-dir uv && uv pip install --system pydantic fastapi uvicorn httpx gradio pytest
...
CMD ["sh", "-lc", "uvicorn translume_ui.app:app --host 0.0.0.0 --port 7860"]
